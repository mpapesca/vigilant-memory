...
import { getDifficultyColor, getDifficultyName } from '@/utils/gameLogic';
    ...
    >
      <View style={styles.levelHeader}>
        ...
        <View style={[styles.difficultyBadge, { backgroundColor: getDifficultyColor(item.difficulty) }]}>
          <Text style={styles.difficultyText}>{getDifficultyName(item.difficulty)}</Text>
        </View>
      </View>
      
      <Text style={[styles.gridInfo, { color: isDark ? '#8e8e93' : '#666666' }]}>
        {item.gridSize.rows}×{item.gridSize.cols} Grid ({item.gridSize.rows * item.gridSize.cols / 2} pairs)
      </Text>
      
      <View style={styles.statsRow}>
        <View style={styles.statItem}>
          <Text style={[styles.statLabel, { color: isDark ? '#8e8e93' : '#666666' }]}>Moves</Text>
          <Text style={[styles.statValue, { color: isDark ? '#ffffff' : '#000000' }]}>{item.moves}</Text>
              ...
              <ScrollView showsVerticalScrollIndicator={false}>
                <View style={styles.modalHeader}>
                  <View>
                    ...
                    <View style={[styles.difficultyBadge, { backgroundColor: getDifficultyColor(selectedLevel.difficulty) }]}>
                      <Text style={styles.difficultyText}>{getDifficultyName(selectedLevel.difficulty)}</Text>
                    </View>
                  </View>
                  <TouchableOpacity
                ...
                <View style={styles.modalStats}>
                  <View style={styles.modalStatItem}>
                    <Text style={[styles.modalStatLabel, { color: isDark ? '#8e8e93' : '#666666' }]}>Grid Size</Text>
                  ...
                  <View style={styles.modalStatItem}>
                    <Text style={[styles.modalStatLabel, { color: isDark ? '#8e8e93' : '#666666' }]}>Moves</Text>
                    <Text style={[styles.modalStatValue, { color: isDark ? '#ffffff' : '#000000' }]}>
                      {selectedLevel.moves}
                    </Text>
                    {selectedLevel.bestMoves && selectedLevel.bestMoves !== selectedLevel.moves && (
                      <Text style={[styles.bestValue, { color: '#28a745' }]}>Best: {selectedLevel.bestMoves}</Text>
                    )}
                  </View>
                  
                  <View style={styles.modalStatItem}>
                    <Text style={[styles.modalStatLabel, { color: isDark ? '#8e8e93' : '#666666' }]}>Time</Text>
                    <Text style={[styles.modalStatValue, { color: isDark ? '#ffffff' : '#000000' }]}>
                      {selectedLevel.timeSpent ? formatTime(selectedLevel.timeSpent) : 'N/A'}
                    </Text>
                    {selectedLevel.bestTime && selectedLevel.bestTime !== selectedLevel.timeSpent && (
                      <Text style={[styles.bestValue, { color: '#28a745' }]}>
